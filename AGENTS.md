...
| **Eigene Skripte mit `loadSubScriptWithOptions(url, { ignoreCache: true })` laden.** `loadSubScript()` bedient sich sonst aus dem Startup-Cache und liefert beim Entwickeln stillschweigend die vorige Fassung — ohne Fehler, ohne Hinweis im Log. Zotero lädt nur `bootstrap.js` selbst mit `ignoreCache`; ab dort ist es unsere Sache. Alternativ Zotero mit `-purgecaches` starten. | `xpcom/plugins.js:205-210` |
